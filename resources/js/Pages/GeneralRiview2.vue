                    ...
                    <div class="row mb-2">
                      <div class="form-grou">
                        ...
                        <select class="form-select" v-model="form.periode">
                          ...
                          <option
                            v-for="(c, index) in dataPeriode"
                            :key="index"
                            :value="c.Id_Periode"
                          >
                            {{ c.Kode_Periode }} ({{ c.Keterangan }})
                          </option>
                        </select>
                      </div>
                    </div>
                    <section id="generalreview">
                            ...
                            <tbody>
                              <tr v-for="(item, index) in data" :key="index">
                                ...
                                >
                                  <div
                                    class="rating"
                                    :data-user="item.category"
                                    :data-criteria="user1.username"
                                  >
                                    <template v-for="i in 5">
                                      <input
                                        type="radio"
                                        :name="`ratings_${item.category}_${user1.username}`"
                                        :value="6 - i"
                                        :id="`rating_${item.category}_${
                                          user1.username
                                        }_${6 - i}`"
                                        class="rating-input"
                                        required
                                        v-model="
                                          form.ratings[
                                            `${item.category}_${user1.username}`
                                          ]
                                        "
                                        @change="
                                          updateCompletion(
      ...
      if (this.completedRatings === this.totalRatings) {
        if (confirm("Apakah Anda yakin ingin mengirim penilaian ini?")) {
          ...
          Object.keys(this.form.ratings).forEach((key) => {
            ...
            formData.ratings[userId][criteriaId] = this.form.ratings[key];
          });
